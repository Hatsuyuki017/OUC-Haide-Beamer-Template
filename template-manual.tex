...
%% Demonstrate the table-of-contents template defined in oucsmartbeamer.sty.
...
\begin{frame}
    ...
    \begin{tabular}{@{}>{\columncolor{paleblue}}p{0.43\textwidth} p{0.43\textwidth}@{}}
        ...
        \texttt{\textbackslash OUCSetPalette\{N\}} & Selects palette 0..23 (brand triple + box backgrounds) \\
...
\begin{frame}[fragile]
    ...
    Call \texttt{\textbackslash OUCSetPalette\{N\}} once in the preamble. Indices 0--23 map to twenty-four predefined palettes; the active name is exposed via \texttt{\textbackslash OUCPaletteName} (current: \OUCPaletteName).
    \begin{lstlisting}[language={[LaTeX]TeX}]
\usepackage{oucsmartbeamer}
\OUCSetPalette{0} %% 0 OUC Default, 1 Brunneophobia, 2 Van Dyke,
                  ...
                  %% 10 Roman Empire, 11 Greece, 12 Kanagawa,
                  %% 13 Starry Night, 14 A Thousand Li,
                  %% 15 And Quiet Flows the Don,
                  %% 16 Cyberpunk Edgerunners,
                  %% 17 The Grand Budapest Hotel,
                  %% 18 Renaissance Florence, 19 Soviet Avant-Garde,
                  %% 20 Constantinople, 21 France, 22 Kyoto,
                  %% 23 Siamese Dream
    \end{lstlisting}
    \begin{infobox}
        Each palette defines six colors \texttt{oucpal1}..\texttt{oucpal6}, sorted light $\rightarrow$ dark. Slot usage: pal1 = lstlisting background, pal2 = Example / Remark / Intro body background, pal4 = Example / Remark / Intro title background, pal5 = \texttt{primary} (\emph{second} darkest; emphasis color for titles, frametitle, footer, theorem titles). pal3 = \texttt{accent} (five-color palettes duplicate pal2 into this slot) and pal6 is the darkest swatch, used for the lstlisting border and free for ad-hoc use. \texttt{spotcolor} follows \texttt{accent}, except under Kyoto (22) where it becomes Shu-hi vermilion. All 24 swatch strips are collected in the \emph{Palette Gallery} at the end of this guide.
    \end{infobox}
\end{frame}

\section{Boxes \& Listings}
...
%% ========== TikZ tutorial: palette-aware drawing =====================
\begin{frame}[fragile]
    \frametitle{TikZ: Palette-Aware Drawing}
    \scriptsize
    \begin{columns}[T]
        \column{0.46\textwidth}
        \centering
        \begin{tikzpicture}[scale=0.7,every node/.style={font=\scriptsize}]
            \fill[paleblue] (0,0) rectangle (4,1.2);
            \draw[primary,thick,rounded corners=2pt]
                (0,0) rectangle (4,1.2);
            \node[fill=primary,text=white,rounded corners=2pt,
                  inner sep=3pt] at (2,0.6) {Palette aware};
            \draw[accent,very thick,-Stealth]
                (0,-0.4) -- (4,-0.4) node[midway,below]{accent arrow};
            \fill[oucpal4] (0.6,-1.3) circle (0.2);
            \fill[oucpal5] (1.5,-1.3) circle (0.2);
            \fill[oucpal6] (2.4,-1.3) circle (0.2);
        \end{tikzpicture}
        \column{0.50\textwidth}
        \begin{lstlisting}[language={[LaTeX]TeX}]
\begin{tikzpicture}[scale=0.75]
  ...
  \node[fill=primary,text=white,
    ...
    at (2,0.7) {Palette aware};
  \draw[accent,very thick,-Stealth]
    (0,-0.6) -- (4,-0.6);
  \fill[oucpal5] (1.4,-1.8) circle (0.25);
\end{tikzpicture}
        \end{lstlisting}
    \end{columns}
    \vspace{2pt}
    {\tiny\textbf{Tip:} use palette-aware names \texttt{primary}, \texttt{accent}, \texttt{paleblue}, \texttt{oucpal1}--\texttt{oucpal6} for auto-recoloring. Pre-loaded: \texttt{positioning}, \texttt{arrows.meta}, \texttt{fit}, \texttt{calc}, \texttt{decorations.pathmorphing}, \texttt{shadows.blur}.}
\end{frame}

%% ========== TikZ tutorial: node diagrams =============================
\begin{frame}[fragile]
    \frametitle{TikZ: Node Diagrams \& Arrows}
    \scriptsize
    \begin{columns}[T]
        \column{0.50\textwidth}
        \centering
        \begin{tikzpicture}[
            ...
            box/.style={draw=primary,fill=paleblue,thick,
                rounded corners=2pt,inner sep=3pt,
                minimum width=14mm,minimum height=7mm,align=center},
            hi/.style={draw=accent,fill=accent!15,thick,
                rounded corners=2pt,inner sep=3pt,
                minimum width=14mm,minimum height=7mm,align=center},
            arr/.style={-Stealth,accent,thick}]
            \node[box] (a) {Input};
            \node[hi,  right=of a] (b) {Process};
...
\begin{tikzpicture}[
  node distance=6mm and 8mm,
  box/.style={draw=primary,fill=paleblue,
    thick,rounded corners=2pt,
    minimum width=14mm,align=center},
  hi/.style ={draw=accent,fill=accent!15,
    thick,rounded corners=2pt,
    minimum width=14mm,align=center},
  arr/.style={-Stealth,accent,thick}]
  \node[box] (a) {Input};
...
%% ========== TikZ tutorial: highlights & annotations ==================
\begin{frame}[fragile]
    \frametitle{TikZ: Highlights \& Annotations}
    \scriptsize
    \begin{columns}[T]
        \column{0.50\textwidth}
        \centering
        \begin{tikzpicture}[every node/.style={font=\scriptsize}]
            ...
            \node (n3) at (2.0,0) {$x_3$};
            \node (n4) at (3.2,0) {$y$};
            \node[draw=accent,thick,rounded corners=2pt,
                  fit=(n1)(n2)(n3),inner sep=4pt,
                  label={[accent,font=\tiny]above:features}] {};
            \draw[primary,thick,-Stealth,
                  decorate,decoration={snake,amplitude=.6pt,
                  segment length=3pt,post length=2pt}]
                (n3.east) -- (n4.west);
            \node[draw=primary,fill=paleblue,thick,
                  rounded corners=2pt,inner sep=2pt,
                  blur shadow={shadow blur steps=4}]
                at (1.5,-1.2) {Result: $y=f(\mathbf{x})$};
        \end{tikzpicture}
        \column{0.46\textwidth}
        \begin{lstlisting}[language={[LaTeX]TeX}]
\begin{tikzpicture}
  ...
  \node[draw=accent,thick,
    rounded corners=2pt,
    fit=(n1)(n2)(n3),inner sep=4pt,
    label=above:features] {};
  \draw[primary,thick,-Stealth,decorate,
    decoration={snake,amplitude=.6pt,
    segment length=3pt}]
    (n3.east) -- (n4.west);
  \node[draw=primary,fill=paleblue,
    rounded corners=2pt,
    blur shadow] at (1.5,-1.2)
    {Result: $y=f(\mathbf{x})$};
\end{tikzpicture}
        \end{lstlisting}
    \end{columns}
...
\section{Palette Gallery}

\begin{frame}
    \frametitle{Palette Gallery}
    All 24 palettes are reproduced on the following pages, each as its full
    \texttt{pal1}--\texttt{pal6} swatch strip with the slot each colour drives.
    Switch to any of them with a single \texttt{\textbackslash OUCSetPalette\{N\}} in the preamble.
    \begin{infobox}
        Indices 0--9 are the original OUC set; 10--23 are the themed palettes shared
        with the Thomas-Tufte-Style-Book-Template, so a deck and its companion book
        can be set in exactly the same colours.
    \end{infobox}
\end{frame}
...
\begin{frame}
    \frametitle{Palette 10 --- Roman Empire}
    \definecolor{pcA}{RGB}{236,232,225}%
    \definecolor{pcB}{RGB}{212,175,55}%
    \definecolor{pcC}{RGB}{74,110,65}%
    \definecolor{pcD}{RGB}{180,30,30}%
    \definecolor{pcE}{RGB}{120,20,40}%
    \definecolor{pcF}{RGB}{88,28,90}%
    \centering\vspace{3pt}
    \begin{tabular}{@{}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{}}
        \fcolorbox{linegray}{pcA}{\rule{0pt}{32pt}\hspace{38pt}} &
        \fcolorbox{linegray}{pcB}{\rule{0pt}{32pt}\hspace{38pt}} &
    ...
    \vspace{4pt}
    \begin{infobox}
        \textbf{Roman Empire} --- legion red and murex purple over imperial marble.
        \hfill\texttt{\textbackslash OUCSetPalette\{10\}}
    \end{infobox}
\end{frame}

\begin{frame}
    \frametitle{Palette 11 --- Greece}
    \definecolor{pcA}{RGB}{245,240,228}%
    \definecolor{pcB}{RGB}{212,175,55}%
    \definecolor{pcC}{RGB}{98,128,48}%
    \definecolor{pcD}{RGB}{188,82,38}%
    \definecolor{pcE}{RGB}{48,105,175}%
    \definecolor{pcF}{RGB}{90,42,92}%
    \centering\vspace{3pt}
    \begin{tabular}{@{}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{}}
        \fcolorbox{linegray}{pcA}{\rule{0pt}{32pt}\hspace{38pt}} &
        \fcolorbox{linegray}{pcB}{\rule{0pt}{32pt}\hspace{38pt}} &
    ...
    \vspace{4pt}
    \begin{infobox}
        \textbf{Greece} --- Aegean blue, olive and terracotta on limestone.
        \hfill\texttt{\textbackslash OUCSetPalette\{11\}}
    \end{infobox}
\end{frame}

\begin{frame}
    \frametitle{Palette 12 --- Kanagawa}
    \definecolor{pcA}{RGB}{237,233,222}%
    \definecolor{pcB}{RGB}{208,224,238}%
    \definecolor{pcC}{RGB}{150,186,210}%
    \definecolor{pcD}{RGB}{26,78,132}%
    \definecolor{pcE}{RGB}{13,38,76}%
    \definecolor{pcF}{RGB}{29,25,35}%
    \centering\vspace{3pt}
    \begin{tabular}{@{}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{}}
        \fcolorbox{linegray}{pcA}{\rule{0pt}{32pt}\hspace{38pt}} &
        \fcolorbox{linegray}{pcB}{\rule{0pt}{32pt}\hspace{38pt}} &
    ...
    \vspace{4pt}
    \begin{infobox}
        \textbf{Kanagawa} --- Edo wave blues graded onto ink-black line work.
        \hfill\texttt{\textbackslash OUCSetPalette\{12\}}
    \end{infobox}
\end{frame}

\begin{frame}
    \frametitle{Palette 13 --- Starry Night}
    \definecolor{pcA}{RGB}{240,208,68}%
    \definecolor{pcB}{RGB}{198,140,52}%
    \definecolor{pcC}{RGB}{105,155,200}%
    \definecolor{pcD}{RGB}{48,96,165}%
    \definecolor{pcE}{RGB}{22,50,30}%
    \definecolor{pcF}{RGB}{20,36,88}%
    \centering\vspace{3pt}
    \begin{tabular}{@{}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{}}
        \fcolorbox{linegray}{pcA}{\rule{0pt}{32pt}\hspace{38pt}} &
        \fcolorbox{linegray}{pcB}{\rule{0pt}{32pt}\hspace{38pt}} &
    ...
    \vspace{4pt}
    \begin{infobox}
        \textbf{Starry Night} --- moon yellow against cypress green and night blue.
        \hfill\texttt{\textbackslash OUCSetPalette\{13\}}
    \end{infobox}
\end{frame}

\begin{frame}
    \frametitle{Palette 14 --- A Thousand Li}
    \definecolor{pcA}{RGB}{218,203,170}%
    \definecolor{pcB}{RGB}{110,165,195}%
    \definecolor{pcC}{RGB}{183,118,45}%
    \definecolor{pcD}{RGB}{96,148,110}%
    \definecolor{pcE}{RGB}{48,105,76}%
    \definecolor{pcF}{RGB}{35,78,112}%
    \centering\vspace{3pt}
    \begin{tabular}{@{}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{}}
        \fcolorbox{linegray}{pcA}{\rule{0pt}{32pt}\hspace{38pt}} &
        \fcolorbox{linegray}{pcB}{\rule{0pt}{32pt}\hspace{38pt}} &
    ...
    \vspace{4pt}
    \begin{infobox}
        \textbf{A Thousand Li} --- blue-and-green landscape mineral pigments on aged silk.
        \hfill\texttt{\textbackslash OUCSetPalette\{14\}}
    \end{infobox}
\end{frame}

\begin{frame}
    \frametitle{Palette 15 --- And Quiet Flows the Don}
    \definecolor{pcA}{RGB}{142,120,70}%
    \definecolor{pcB}{RGB}{110,108,104}%
    \definecolor{pcC}{RGB}{118,86,52}%
    \definecolor{pcD}{RGB}{122,66,36}%
    \definecolor{pcE}{RGB}{52,70,90}%
    \definecolor{pcF}{RGB}{88,26,24}%
    \centering\vspace{3pt}
    \begin{tabular}{@{}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{}}
        \fcolorbox{linegray}{pcA}{\rule{0pt}{32pt}\hspace{38pt}} &
        \fcolorbox{linegray}{pcB}{\rule{0pt}{32pt}\hspace{38pt}} &
    ...
    \vspace{4pt}
    \begin{infobox}
        \textbf{And Quiet Flows the Don} --- turbid steppe browns with a cold blue and dried blood.
        \hfill\texttt{\textbackslash OUCSetPalette\{15\}}
    \end{infobox}
\end{frame}

\begin{frame}
    \frametitle{Palette 16 --- Cyberpunk Edgerunners}
    \definecolor{pcA}{RGB}{225,255,8}%
    \definecolor{pcB}{RGB}{10,238,100}%
    \definecolor{pcC}{RGB}{238,18,120}%
    \definecolor{pcD}{RGB}{205,20,35}%
    \definecolor{pcE}{RGB}{15,32,98}%
    \definecolor{pcF}{RGB}{14,8,28}%
    \centering\vspace{3pt}
    \begin{tabular}{@{}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{}}
        \fcolorbox{linegray}{pcA}{\rule{0pt}{32pt}\hspace{38pt}} &
        \fcolorbox{linegray}{pcB}{\rule{0pt}{32pt}\hspace{38pt}} &
    ...
    \vspace{4pt}
    \begin{infobox}
        \textbf{Cyberpunk Edgerunners} --- maximum-voltage neon over Night City black.
        \hfill\texttt{\textbackslash OUCSetPalette\{16\}}
    \end{infobox}
\end{frame}

\begin{frame}
    \frametitle{Palette 17 --- The Grand Budapest Hotel}
    \definecolor{pcA}{RGB}{247,235,215}%
    \definecolor{pcB}{RGB}{168,178,196}%
    \definecolor{pcC}{RGB}{237,148,158}%
    \definecolor{pcD}{RGB}{176,136,60}%
    \definecolor{pcE}{RGB}{128,88,148}%
    \definecolor{pcF}{RGB}{143,52,65}%
    \centering\vspace{3pt}
    \begin{tabular}{@{}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{}}
        \fcolorbox{linegray}{pcA}{\rule{0pt}{32pt}\hspace{38pt}} &
        \fcolorbox{linegray}{pcB}{\rule{0pt}{32pt}\hspace{38pt}} &
    ...
    \vspace{4pt}
    \begin{infobox}
        \textbf{The Grand Budapest Hotel} --- faded aristocratic pinks and violets on a cream ground.
        \hfill\texttt{\textbackslash OUCSetPalette\{17\}}
    \end{infobox}
\end{frame}

\begin{frame}
    \frametitle{Palette 18 --- Renaissance Florence}
    \definecolor{pcA}{RGB}{238,228,208}%
    \definecolor{pcB}{RGB}{190,148,52}%
    \definecolor{pcC}{RGB}{172,84,50}%
    \definecolor{pcD}{RGB}{54,80,60}%
    \definecolor{pcE}{RGB}{46,70,118}%
    \definecolor{pcF}{RGB}{85,54,34}%
    \centering\vspace{3pt}
    \begin{tabular}{@{}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{}}
        \fcolorbox{linegray}{pcA}{\rule{0pt}{32pt}\hspace{38pt}} &
        \fcolorbox{linegray}{pcB}{\rule{0pt}{32pt}\hspace{38pt}} &
    ...
    \vspace{4pt}
    \begin{infobox}
        \textbf{Renaissance Florence} --- lapis, gold leaf and terracotta in fresco proportions.
        \hfill\texttt{\textbackslash OUCSetPalette\{18\}}
    \end{infobox}
\end{frame}

\begin{frame}
    \frametitle{Palette 19 --- Soviet Avant-Garde}
    \definecolor{pcA}{RGB}{225,218,205}%
    \definecolor{pcB}{RGB}{200,150,32}%
    \definecolor{pcC}{RGB}{118,114,108}%
    \definecolor{pcD}{RGB}{50,80,138}%
    \definecolor{pcE}{RGB}{196,28,28}%
    \definecolor{pcF}{RGB}{24,20,18}%
    \centering\vspace{3pt}
    \begin{tabular}{@{}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{}}
        \fcolorbox{linegray}{pcA}{\rule{0pt}{32pt}\hspace{38pt}} &
        \fcolorbox{linegray}{pcB}{\rule{0pt}{32pt}\hspace{38pt}} &
    ...
    \vspace{4pt}
    \begin{infobox}
        \textbf{Soviet Avant-Garde} --- constructivist red and black with industrial greys.
        \hfill\texttt{\textbackslash OUCSetPalette\{19\}}
    \end{infobox}
\end{frame}

\begin{frame}
    \frametitle{Palette 20 --- Constantinople}
    \definecolor{pcA}{RGB}{236,225,207}%
    \definecolor{pcB}{RGB}{195,150,42}%
    \definecolor{pcC}{RGB}{170,112,50}%
    \definecolor{pcD}{RGB}{50,76,58}%
    \definecolor{pcE}{RGB}{102,32,65}%
    \definecolor{pcF}{RGB}{30,52,88}%
    \centering\vspace{3pt}
    \begin{tabular}{@{}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{}}
        \fcolorbox{linegray}{pcA}{\rule{0pt}{32pt}\hspace{38pt}} &
        \fcolorbox{linegray}{pcB}{\rule{0pt}{32pt}\hspace{38pt}} &
    ...
    \vspace{4pt}
    \begin{infobox}
        \textbf{Constantinople} --- Byzantine gilt, porphyry and the deep sea of the capital.
        \hfill\texttt{\textbackslash OUCSetPalette\{20\}}
    \end{infobox}
\end{frame}

\begin{frame}
    \frametitle{Palette 21 --- France}
    \definecolor{pcA}{RGB}{232,222,205}%
    \definecolor{pcB}{RGB}{142,133,58}%
    \definecolor{pcC}{RGB}{122,126,132}%
    \definecolor{pcD}{RGB}{143,108,148}%
    \definecolor{pcE}{RGB}{180,32,40}%
    \definecolor{pcF}{RGB}{44,74,138}%
    \centering\vspace{3pt}
    \begin{tabular}{@{}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{}}
        \fcolorbox{linegray}{pcA}{\rule{0pt}{32pt}\hspace{38pt}} &
        \fcolorbox{linegray}{pcB}{\rule{0pt}{32pt}\hspace{38pt}} &
    ...
    \vspace{4pt}
    \begin{infobox}
        \textbf{France} --- the tricolour reread through muted archival tones.
        \hfill\texttt{\textbackslash OUCSetPalette\{21\}}
    \end{infobox}
\end{frame}

\begin{frame}
    \frametitle{Palette 22 --- Kyoto}
    \definecolor{pcA}{RGB}{234,226,212}%
    \definecolor{pcB}{RGB}{204,184,178}%
    \definecolor{pcC}{RGB}{116,126,120}%
    \definecolor{pcD}{RGB}{110,84,64}%
    \definecolor{pcE}{RGB}{80,94,68}%
    \definecolor{pcF}{RGB}{34,40,66}%
    \centering\vspace{3pt}
    \begin{tabular}{@{}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{}}
        \fcolorbox{linegray}{pcA}{\rule{0pt}{32pt}\hspace{38pt}} &
        \fcolorbox{linegray}{pcB}{\rule{0pt}{32pt}\hspace{38pt}} &
    ...
    \vspace{4pt}
    \begin{infobox}
        \textbf{Kyoto} --- zen stillness and \emph{y\=ugen}; carries the Shu-hi \texttt{spotcolor}.
        \hfill\texttt{\textbackslash OUCSetPalette\{22\}}
    \end{infobox}
\end{frame}

\begin{frame}
    \frametitle{Palette 23 --- Siamese Dream}
    \definecolor{pcA}{RGB}{238,226,208}%
    \definecolor{pcB}{RGB}{182,180,193}%
    \definecolor{pcC}{RGB}{198,158,65}%
    \definecolor{pcD}{RGB}{150,145,135}%
    \definecolor{pcE}{RGB}{188,105,40}%
    \definecolor{pcF}{RGB}{42,68,48}%
    \centering\vspace{3pt}
    \begin{tabular}{@{}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{\hspace{2pt}}c@{}}
        \fcolorbox{linegray}{pcA}{\rule{0pt}{32pt}\hspace{38pt}} &
        \fcolorbox{linegray}{pcB}{\rule{0pt}{32pt}\hspace{38pt}} &
    ...
    \vspace{4pt}
    \begin{infobox}
        \textbf{Siamese Dream} --- overexposed haze, slow-burn orange and organic dark green.
        \hfill\texttt{\textbackslash OUCSetPalette\{23\}}
    \end{infobox}
\end{frame}
